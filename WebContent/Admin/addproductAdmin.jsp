...
<body>
	...
	<form action="ProductController?command=addproduct" method="post" accept-charset="UTF-8">
		제품종류 : <select class = "form-control" name="type">
			<c:forEach var="ceteList" items="${cateList}">
				<option value="${ceteList }">${ceteList }</option>
			</c:forEach>
				</select></br>
		제품이름: <input type="text" name="name" class = "form-control"/><br />
		가격: <input type="text" name="price" class = "form-control"/><br /> 
		설명: <input type="text" name="explain" class = "form-control"/><br />
		상세설명: <input type="text" name="detailexplain" class = "form-control"/><br /> 
		이미지: <input type="text" name="img" class = "form-control"/><br /> 
		...
		<div>
			<input type="submit" class="btn btn-info fr" value="작성" /> 
			<input type="button" value="뒤로" class="btn btn-primary"
