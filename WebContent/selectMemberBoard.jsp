	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			<c:forEach var="b" items="${list}">
				<tr>
					<td>${b.member_id}</a></td>
					<td><a href="NikeactionController?command=boardNumber&boardNumber=${b.member_id}">${b.id}</a></td>
					<!-- 컨틀롤러 -->
					<td>${b.pw}</td>
					<td>${b.name}</td>
					<td>${b.address}</td>
            ...
            <td colspan='8'>
               <%
                  if(currentPage >= 10){
               %>
                  <a href="NikeBulletinBoardController?command=select&currentPage=<%=currentPage-10%>">[이전페이지]</a>               
               <%      
                  }
               //endPage
               ...
               %> <a href="NikeBulletinBoardController?command=select&currentPage=<%=a%>">[<%=a%>]</a>
               <%
                      }
                   }
                if(currentPage != maxPage){
                %>
                   <a href="NikeBulletinBoardController?command=select&currentPage=<%=currentPage+10%>">[다음페이지]</a>
                <%      
                }
               %>
            </td>
